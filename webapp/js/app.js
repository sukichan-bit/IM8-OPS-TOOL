...
// Small colored pill for "Dispatched" (green) / "Partially Dispatched"
// (amber) / "Not Dispatched" (pink) — same palette as the row-green/
// row-amber/row-red table highlights, so status reads consistently in
// prose, headers, and table rows alike.
function statusChip(status) {
  const cls =
    status === "Dispatched" ? "status-chip-dispatched" : status === "Partially Dispatched" ? "status-chip-partial" : "status-chip-not-dispatched";
  return h("span", {
...
async function handleTaskCOpenSo(file) {
  ...
  if (refundedOnSheet) {
    ...
    // Real WH04/on-hand exports proved SheetJS's own numeric-to-Date
    // conversion is off by a fixed, file-independent 28,842s (8h 0m 42s) for
    // certain date cells — reproduced across 350+ real cells in two separate
    // Open SO workbooks with 100% consistency (see recomputeSheetDatesFromRawSerials).
    // Recompute this tab's dates from their own raw serials so refund-date
    // resolution isn't quietly off by a day.
    const fullRaw = io.recomputeSheetDatesFromRawSerials(buf, file.name, refundedOnSheet, io.sheetToRawRows(wb, refundedOnSheet, null));
  ...
  const actionsRaw = actionsSheet
    ? io.recomputeSheetDatesFromRawSerials(buf, file.name, actionsSheet, io.sheetToRawRows(wb, actionsSheet, null))
    : null;
  ...
  if (workingsSheet) {
    ...
    const workingsFullRaw = io.recomputeSheetDatesFromRawSerials(buf, file.name, workingsSheet, io.sheetToRawRows(wb, workingsSheet, null));
    workingsRows = io.loadTableFromRawRows(workingsFullRaw, g.row);
...
function renderTaskC() {
  ...
  const entity = inferFacility(known);
  let combinedPerSheet = {};
  const combinedDiagnostics = {};
  const covered = new Set();
  let allOutRows = [];
  ...
  // Steps 5-7 further below need "shipped at least something" — Partially
  // Dispatched orders still need Step 5's D365 fulfillment template (for
  // the units that did ship, flagged as partial) and must NOT fall into
  // Step 6's never-shipped inventory-adjustment journal.
  const dispatchedRows = allOutRows.filter((r) => r.__dispatch_status !== "Not Dispatched");
  ...
  const STATUS_ORDER = ["Dispatched", "Partially Dispatched", "Not Dispatched"];
  const statusRank = (s) => { const i = STATUS_ORDER.indexOf(s); return i === -1 ? STATUS_ORDER.length : i; };
  const statusCounts = STATUS_ORDER.map((status) => {
    const rows = allOutRows.filter((r) => r.__dispatch_status === status);
    return { status, orderCount: new Set(rows.map(orderKeyOf)).size, lineCount: rows.length };
  }).filter((s) => s.lineCount > 0);
  const totalOrders = statusCounts.reduce((s, c) => s + c.orderCount, 0);
  const captionParts = [`${totalOrders} order(s) checked — `];
  statusCounts.forEach((c, i) => {
    if (i > 0) captionParts.push(", ");
    captionParts.push(`${c.orderCount} `, statusChip(c.status), ` (${c.lineCount} line(s))`);
  });
  captionParts.push(".");
  root.appendChild(h("p", { class: "caption" }, captionParts));
  root.appendChild(dispatchArea);

  // One combined workbook, one tab per warehouse + status — Dispatched/
  // Partially Dispatched rows grouped by their actual Shipped-date month,
  // Not Dispatched rows by their resolved Refund-date month (mirroring
  // Step 6's journal splitting); a (warehouse, status) pair only gets a
  // " - Mon" suffix on its tab name when it actually spans more than one
  // month — otherwise the month is implied and left off. A status with no
  // rows at all for a warehouse simply never gets a tab (nothing to skip).
  const monthKeyForDispatchRow = (row) => {
    const raw =
      row.__dispatch_status !== "Not Dispatched"
  ...
  const monthsPerWhStatus = new Map(); // `${wh}|${status}` -> Set(monthKey)
  for (const g of sortedGroups) {
    const k = `${g.wh}|${g.status}`;
    if (!monthsPerWhStatus.has(k)) monthsPerWhStatus.set(k, new Set());
    monthsPerWhStatus.get(k).add(g.monthKey);
  }
  const sheetDict = {};
  for (const group of sortedGroups) {
    const needsMonth = monthsPerWhStatus.get(`${group.wh}|${group.status}`).size > 1;
    const sheetName = needsMonth ? `${group.wh} (${group.status}) - ${shortMonthLabel(group.monthKey)}` : `${group.wh} (${group.status})`;
    sheetDict[sheetName] = group.rows;
  }
  root.appendChild(
    h("p", {
      class: "file-desc",
      text: "One combined file, one tab per warehouse + status (+ month, only where a warehouse/status has more than one) — the raw dispatch-check detail (which orders matched, tracking/shipped date found or not). Use it to hand off or archive; the overview below lets you preview any of these without downloading.",
    })
  );
  const dispatchBtn = h("button", { text: `Download Dispatch Check — ${sortedGroups.length} tab(s) (.xlsx)` });
  dispatchBtn.addEventListener("click", async () => {
    dispatchBtn.disabled = true;
    const original = dispatchBtn.textContent;
    dispatchBtn.textContent = "Building file...";
    try {
      const buf = await io.toExcelBytes(sheetDict);
      downloadBlob(new Blob([buf], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }), `${todayStamp()} ${entity} refund-cancel order_Dispatch check.xlsx`);
    } finally {
      dispatchBtn.disabled = false;
      dispatchBtn.textContent = original;
    }
  });
  dispatchArea.appendChild(dispatchBtn);

  // A-Z by warehouse (e.g. USOPS-WH04 before USOPS-WH05), Dispatched before
  // Partially Dispatched before Not Dispatched within each warehouse.
  ...
  step5Btn.addEventListener("click", async () => {
    ...
    try {
      ...
      downloadBlob(new Blob([buf], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }), `${todayStamp()} ${entity} refund order D365 fulfillment_dispatched.xlsx`);
    } finally {
  ...
  for (const file of sortedJournalFiles) {
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        downloadBlob(new Blob([buf], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }), `${todayStamp()} ${entity} refund order_D365 adjustment journal lines - ${file.warehouse} ${file.monthKey}.xlsx`);
      } finally {
        btn.disabled = false;
  ...
  step7Btn.addEventListener("click", async () => {
    ...
    try {
      ...
      downloadBlob(new Blob([buf], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }), `${todayStamp()} ${entity} refund order D365 fulfillment_not dispatched.xlsx`);
    } finally {
...
function renderQuoteBreakdown(container, quote, card, warehouseName) {
  ...
  if (quote.split) {
    ...
    if (quote.overageApplied) {
      container.appendChild(h("p", { class: "warning", text: `⚠ This shipment exceeds the rate card's maximum weight — priced as the top weight bracket plus this card's documented over-max overage rate (${fmtMoney(quote.overageAmount)}), not a split into multiple parcels. Per Stord's own policy this overage rate is meant for a shipment whose ACTUAL weight is within the card's max but whose dimensional/billable weight pushes it over — if the actual weight itself is well over the max, confirm with Stord that this service can even accept the shipment before quoting a customer.` }));
    }
...
function renderTaskGComparisonTable(container, results) {
  ...
  for (const r of results) {
    ...
    if (r.error) {
      ...
      if (r.overageApplied) notes.push(`over max weight — includes a ${fmtMoney(r.overageAmount)} overage surcharge (confirm with the carrier if actual weight, not just billable weight, is well over the max)`);
