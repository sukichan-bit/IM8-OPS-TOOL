...
// A bundle name like "IM8-FG-000127 & IM8-FG-000198 (virtual bundle)"
// already names its own components — parse them directly instead of always
// requiring the user to re-type SKUs the name already tells us. Only fires
// when EVERY "&"-joined token looks like a real item code (this codebase's
// convention: an "IM8-" prefix); a friendly/marketing bundle name (e.g.
// "Starter Kit & Travel Pack (virtual bundle)") still falls through to the
// manual composition UI, since there's nothing safe to parse there.
const BUNDLE_NAME_ITEM_RE = /^IM8-[A-Z0-9-]+$/i;

function tryParseBundleNameComponents(name) {
  const stripped = String(name || "").replace(VIRTUAL_BUNDLE_RE, "").trim();
  if (!stripped) return null;
  const tokens = stripped.split(/\s*&\s*/).map((t) => t.trim()).filter(Boolean);
  if (tokens.length < 2 || !tokens.every((t) => BUNDLE_NAME_ITEM_RE.test(t))) return null;
  return tokens.map((item) => ({ item, productName: "" }));
}
...
function buildFgLines(itemRecords, warehouse, location, bundleCompositions) {
  ...
  for (const rec of itemRecords) {
    ...
    if (isVirtualBundleName(rec.sku) || isVirtualBundleName(rec.productName)) {
      ...
      let bundle = bundleCompositions && bundleCompositions[key];
      if (!bundle || !bundle.composition || !bundle.composition.length) {
        const parsed = tryParseBundleNameComponents(nameOrSku);
        if (parsed) {
          bundle = { composition: parsed };
        } else {
